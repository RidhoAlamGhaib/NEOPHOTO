...
new class extends Component
{
    ...
    // ── ID/VISA passport requirement lookup ─────────────────────────────────────
    private const IDVISA_PRODUCT_IDS = [13]; // GANTI sesuai product_id asli ID/VISA
    ...
    // ── ID/VISA: simpen negara yang dipilih ke item (kayak addon) ──────────────
    public function selectCountry($orderItemId, $country)
    {
    ...
    // ── ID/VISA: ambil requirement negara buat ditampilin di receipt ───────────
    public function getPassportRequirements(?string $country)
    {
        if (!$country) {
            return collect();
        }

        return \App\Models\PassportPhotoSize::where('country', $country)->get();
    }
    ...
    public function mount($id)
    {
        ...
        $this->countries = \App\Models\PassportPhotoSize::select('country')
            ->distinct()
            ->orderBy('country')
            ->pluck('country');
    }
                            ...
                            <div class="accordion-item">
                                ...
                                <div id="flush-{{ $item->product->type }}"
                                    ...
                                    <div style="background-color:#fab7be"
                                        ...
                                        @if (in_array($item->product_id, self::IDVISA_PRODUCT_IDS))
                                            {{-- ── ID/VISA: "Pilih Negara" diperlakukan kayak addon ── --}}
                                            <div class="mb-3 text-start">
                                                <label class="form-label text-white fw-bold mb-1" style="font-size:13px;">
                                                    Pilih Negara
                                                </label>
                                                <select wire:change="selectCountry({{ $item->id }}, $event.target.value)"
                                                        class="form-select form-select-sm shadow-sm"
                                                        style="border-radius:10px;border:1px solid rgba(255,255,255,0.6);padding:8px 12px;">
                                                    <option value="">-- Pilih Negara --</option>
                                                    @foreach ($countries as $c)
                                                        <option value="{{ $c }}" @selected($item->country === $c)>{{ $c }}</option>
                                                    @endforeach
                                                </select>
                                            </div>

                                            @if ($item->country)
                                                @php $reqs = $this->getPassportRequirements($item->country); @endphp
                                                <div class="mb-2 p-2 rounded text-start text-white"
                                                     style="background:rgba(0,0,0,0.18);font-size:12px;">
                                                    <div class="fw-bold mb-1 text-white">NEGARA: {{ $item->country }}</div>
                                                    @forelse ($reqs as $r)
                                                        <div class="mb-2 text-white {{ !$loop->last ? 'pb-2 border-bottom' : '' }}"
                                                             style="border-color:rgba(255,255,255,0.3) !important;">
                                                            <div class="fw-bold text-uppercase text-white" style="font-size:11px;opacity:.85;">
                                                                {{ str_replace('_', ' ', $r->document_type) }}
                                                            </div>
                                                            <div class="text-white">UKURAN: {{ $r->width_cm }} × {{ $r->height_cm }} cm
                                                                ({{ $r->width_px }} × {{ $r->height_px }} px)</div>
                                                            <div class="text-white">BACKGROUND: {{ $r->background ?? '-' }}</div>
                                                            <div class="text-white">Min: {{ $r->min_copies }} lembar</div>
                                                        </div>
                                                    @empty
                                                        <span class="text-white">Requirement belum ada di database.</span>
                                                    @endforelse
                                                </div>
                                            @endif
                                        @else
                                    ...
                                    <div class="card p-4">
                                        ...
                                        @foreach ($order->items as $item)
                                            <div class="border-bottom pb-2">
                                                ...
                                                @if (in_array($item->product_id, self::IDVISA_PRODUCT_IDS) && $item->country)
                                                    @php $reqs = $this->getPassportRequirements($item->country); @endphp
                                                    <div class="ms-3 mb-2 p-2 rounded border" style="background:#f8f9fa;font-size:11px;">
                                                        @forelse ($reqs as $r)
                                                            <table class="table table-sm table-borderless mb-1 text-start" style="font-size:11px;">
                                                                <tbody>
                                                                    <tr>
                                                                        <td class="fw-bold ps-0" style="width:35%;">NEGARA</td>
                                                                        <td>: {{ $item->country }}</td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td class="fw-bold ps-0">UKURAN</td>
                                                                        <td>: {{ $r->width_cm }} × {{ $r->height_cm }} cm
                                                                            ({{ $r->width_px }} × {{ $r->height_px }} px)</td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td class="fw-bold ps-0">BACKGROUND</td>
                                                                        <td>: {{ $r->background ?? '-' }}</td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td class="fw-bold ps-0">JENIS</td>
                                                                        <td>: {{ ucfirst(str_replace('_', ' ', $r->document_type)) }} (min {{ $r->min_copies }} lembar)</td>
                                                                    </tr>
                                                                </tbody>
                                                            </table>
                                                            @if ($r->notes)
                                                                <div class="text-muted mb-2" style="font-size:10px;"> {{ $r->notes }}</div>
                                                            @endif
                                                            @if (!$loop->last)
                                                                <hr class="my-2">
                                                            @endif
                                                        @empty
                                                            <span class="text-muted">Requirement belum ada di database buat negara ini.</span>
                                                        @endforelse
                                                    </div>
                                                @endif
